...
// POST { fields: { Name, Email, password, Role } } | PATCH { recordId, fields }
...
exports.handler = async function (event) {
  ...
  if (staff.role !== 'Admin')
    return { statusCode: 403, headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ error: 'Admin only' }) };
  let body;
  ...
  const fields = { ...rest };
  if (isCreate) {
    if (!password) return { statusCode: 400, headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ error: 'Password required' }) };
    if (!fields.Email) return { statusCode: 400, headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ error: 'Email required' }) };
