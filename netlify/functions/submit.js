...
exports.handler = async function (event) {
  ...
  // Airtable field names for the two photo fields
  ...
  for (const { dataKey, fileName, fieldName } of attachmentFields) {
    ...
    if (!base64) continue; // skip if user didn't attach this file
  ...
  // ── Return success (record was created even if attachments failed) ──────────
  return {
    statusCode: 200,
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
